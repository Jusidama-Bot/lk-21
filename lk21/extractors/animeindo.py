...
class Animeindo(BaseExtractor):
    ...
    def extract_data(self, id: str) -> dict:
        ...
        for p in soup.findAll("p", class_="has-text-align-center"):
            if aa := p.findAll("a"):
                info = [
                    self.re.split(r"\s*:", i)[0]
                    for i in self.re.findall(r">\s*(\d+p(?:\s+.+?)?)\s*:", str(p))
                ]
                if len(info) == 1:
                    d = {info[0]: {a.text: a["href"] for a in aa}}
                else:
                    formats.pop(0)
                    d = {}
                    for sub, dom in self.re.findall(
                        r"(\d+p.+?):.+?((?:<a.+?>.+?</a>(?:\s*\|\s*)?)+)", str(p)
                    ):
                        aa = self.soup(dom).findAll("a")
                        d[sub] = {a.text: a["href"] for a in aa}
                result[formats[0]] = d
        return result
