...
def ArgumentParser(**kwargs):
    ...
    parser.add_argument(
        "query", metavar="query", nargs="*", help="kueri, judul, kata kunci"
    )
    ...
    parser.add_argument(
        ...
        dest="page",
        help=(
            "halaman situs, contoh penggunaan:\n"
            "  - 1,2,3\n"
            "  - 1:2, 2:8\n"
            "  - 1,2,3:8\n"
    ...
    parser.add_argument(
        ...
        dest="info",
        action="store_true",
        help="cetak informasi dari item yang dipilih",
    )
    parser.add_argument(
        "--exec",
        metavar="cmd",
        dest="_exec",
        help=(
            "jalankan 'perintah' dengan argument berupa\n"
            "url yang dipilih.\n\n"
            "contoh: aria2c {}"
        ),
    )

    valid_e_rules = re.compile(r"\[\^.+?\][+*]")
    ...
    parser.add_argument(
        "--json", action="store_true", help="cetak hasil ekstraksi unduhan"
    )

    network = parser.add_argument_group("Network Options")
    network.add_argument(
        "--proxy", metavar="IP:PORT", type=ipPortType, help="gunakan HTTP/HTTPS proxy"
    )
    network.add_argument(
        "--skip-proxy", action="store_true", help="lewati penggunakan argument --proxy"
    )

    extractor_group = parser.add_argument_group(
        "Daftar Ekstraktor",
        description=(f"pilih salah satu dari {len(extractors)} situs berikut:"),
    )
    ...
    extractor_exclusiveGroup.add_argument(
        "-a",
        "--all",
        help="Gunakan semua ekstraktor yg tersedia,\nhanya menampilkan hasil yang relevan",
        action="store_true",
    )
        ...
        if hasattr(kls, "host") and getattr(kls, "tag", None):
            ...
            for fullName in [extractorName, re.sub(r"[aioue-]", "", extractorName)]:
                for index in range(0, len(fullName), 2):
                    for shortName in (
                        fullName[index : index + 1],
                        fullName[index : index + 2],
                    ):
